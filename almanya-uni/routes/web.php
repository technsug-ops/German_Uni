...
$routes = function () {
    ...
    Route::post('/chat', function (\Illuminate\Http\Request $request) {
        @set_time_limit(120);
        // locale closure-param olarak enjekte EDİLMİYOR (ArgumentCountError) →
        // SetLocale middleware'inin ayarladığı aktif locale'i kullan.
...
Route::middleware('auth')->group(function () {
    ...
    // Blog (TR) faktlerini Gemini + Google arama ile doğrula (rapor). Blog butonuyla
    // üretilen taslakların yanlış bilgisi yayınlanmasın. ⚠️ işaretlileri /admin/posts'ta düzelt.
    //   ?limit=10  ?all=1 (yayındakiler dahil)
    Route::get('/admin/ops/blog-verify', function () {
        abort_unless(auth()->user()?->is_admin, 403);
        @set_time_limit(600);
        @ini_set('max_execution_time', '600');
        try {
    ...
    Route::get('/admin/ops/kb-embed-reddit', function () {
        ...
        try {
            ...
            if (request()->boolean('reurl')) $args['--reurl'] = true;
            if (request()->boolean('dry')) $args['--dry-run'] = true;
...
// NOT: Merge edilen alt-birim slug'larının 301'i burada Route::redirect ile denendi ama
// hiç çalışmadı — üni sayfaları Route::prefix('{locale}') grubunda ve /universities/{slugOrId}
// daha önce tanımlı olduğu için ilk eşleşen o oluyordu (sonuç 404). Yönlendirme artık
// UniversityWebController::MERGED_SLUGS içinde.
