...
class Integrations extends Page
{
    ...
    public function testFlatReklam(): void
    {
        ...
        try {
            ...
            $info = $svc->siteInfo();
            $total = $svc->list(null, 1, 1, null, 'published')['total'];

            // Hangi token yolları hazır?
            $hasSettingToken = (string) Setting::get('flatreklam_api_token') !== '';
            $activeClients = \App\Models\ApiClient::where('is_active', true)->count();

            $tokenLine = $hasSettingToken
                ? "Token: ✓ Entegrasyon ayar token'ı kayıtlı"
                : "Token: API İstemcileri'nden üret (aktif istemci: {$activeClients})";

            Notification::make()
                ->title('✅ FlatReklam SEO API hazır')
                ->body("Profil: {$info['siteProfile']} · {$total} SEO kaynağı (blog/etkinlik/yasal)\n\n"
                    . "Base URL (panele gir):\n{$base}\n\n"
                    . "{$tokenLine}\n\n"
                    . "Token için: ya buradaki FlatReklam alanına Üret+Kaydet, ya da /admin → API İstemcileri → 'Token üret'. İkisi de geçerli.")
                ->success()->persistent()->send();
        } catch (\Throwable $e) {
            Notification::make()->title('❌ Hata')->body($e->getMessage())->danger()->persistent()->send();
        }
    }
