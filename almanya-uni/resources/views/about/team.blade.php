...
<section class="relative bg-gradient-to-br from-indigo-700 via-purple-600 to-pink-500 text-white overflow-hidden">
    ...
    <div class="relative max-w-[1400px] mx-auto px-4 py-14 md:py-20">
        ...
        <div class="grid md:grid-cols-[1fr_auto] gap-8 items-center">
            <div>
                <span class="inline-flex items-center gap-1.5 px-3 py-1 mb-3 rounded-full bg-white/15 backdrop-blur-sm text-xs font-bold uppercase tracking-wider ring-1 ring-white/25"><x-svg-icon name="sparkles" class="w-3.5 h-3.5" /> {{ __('Meet the team') }}</span>
...
<div x-data="{ open: false, author: {} }" class="max-w-[1400px] mx-auto px-4 py-12 space-y-14">
    ...
    @if ($founders->isNotEmpty())
        <section>
            <div class="flex items-center gap-3 mb-6">
                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gradient-to-r from-amber-400 to-orange-500 text-white text-xs font-extrabold uppercase tracking-wider shadow-md"><x-svg-icon name="trophy" class="w-3.5 h-3.5" /> {{ __('Founder') }}</span>
                    ...
                    <article @click="author = @js($authorData($p, $colors[$i % count($colors)])); open = true"
                        ...
                        <div class="relative flex flex-col sm:flex-row gap-6 items-start sm:items-center">
                            ...
                            <div class="relative shrink-0">
                                ...
                                @endif
                                <span class="absolute -top-2 -right-2 inline-flex items-center justify-center w-8 h-8 md:w-9 md:h-9 rounded-full bg-amber-400 text-white drop-shadow-md group-hover:rotate-12 transition"><x-svg-icon name="trophy" class="w-4 h-4 md:w-5 md:h-5" /></span>
                            </div>
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex flex-wrap gap-3 mt-4 items-center">
                                    @if ($p->posts_count > 0)
                                        <span class="inline-flex items-center gap-1 text-xs font-bold px-3 py-1 rounded-full bg-amber-100 text-amber-900"><x-svg-icon name="pencil" class="w-3 h-3" /> {{ __(':count posts', ['count' => $p->posts_count]) }}</span>
                                    @endif
                                    @if ($p->slug)
                                        <a href="{{ route('author.show', $p->slug) }}" @click.stop
                                           class="inline-flex items-center gap-1 text-xs font-bold px-3 py-1 rounded-full bg-indigo-600 text-white hover:bg-indigo-700 transition">
                                            <x-svg-icon name="newspaper" class="w-3 h-3" /> {{ __('View profile') }} →
                                        </a>
                                    @endif
                                    ...
                                    @php
                                        ...
                                        $iconMap = [
                                            'linkedin' => ['icon' => 'briefcase', 'prefix' => 'https://linkedin.com/in/'],
                                            'twitter'  => ['icon' => 'chat-bubble', 'prefix' => 'https://x.com/'],
                                            'x'        => ['icon' => 'chat-bubble', 'prefix' => 'https://x.com/'],
                                            'github'   => ['icon' => 'computer', 'prefix' => 'https://github.com/'],
                                            'website'  => ['icon' => 'globe', 'prefix' => ''],
                                            'url'      => ['icon' => 'globe', 'prefix' => ''],
                                            'email'    => ['icon' => 'envelope', 'prefix' => 'mailto:'],
                                        ];
                                    @endphp
                                    @foreach ($socials as $type => $val)
                                        ...
                                        <a href="{{ $url }}" target="_blank" rel="noopener noreferrer me" @click.stop
                                           class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-white/80 hover:bg-white border border-gray-200 text-gray-600 transition shadow-sm"
                                           title="{{ ucfirst($type) }}">
                                            <x-svg-icon :name="$m['icon']" class="w-4 h-4" />
                                        </a>
                                    @endforeach
                                </div>
                            </div>
    ...
    @if ($editors->isNotEmpty())
        @php
            ...
            $themes = [
                ['from' => 'from-rose-400',    'to' => 'to-pink-500',    'bg' => 'from-rose-50/80 to-pink-50',     'ring' => 'ring-rose-200',    'badge' => 'bg-rose-100 text-rose-800',       'icon' => 'paint-brush'],
                ['from' => 'from-sky-400',     'to' => 'to-indigo-500',  'bg' => 'from-sky-50/80 to-indigo-50',    'ring' => 'ring-sky-200',     'badge' => 'bg-sky-100 text-sky-800',         'icon' => 'globe'],
                ['from' => 'from-emerald-400', 'to' => 'to-teal-500',    'bg' => 'from-emerald-50/80 to-teal-50',  'ring' => 'ring-emerald-200', 'badge' => 'bg-emerald-100 text-emerald-800', 'icon' => 'leaf'],
                ['from' => 'from-amber-400',   'to' => 'to-orange-500',  'bg' => 'from-amber-50/80 to-orange-50',  'ring' => 'ring-amber-200',   'badge' => 'bg-amber-100 text-amber-800',     'icon' => 'sparkles'],
                ['from' => 'from-violet-400',  'to' => 'to-purple-500',  'bg' => 'from-violet-50/80 to-purple-50', 'ring' => 'ring-violet-200',  'badge' => 'bg-violet-100 text-violet-800',   'icon' => 'star'],
                ['from' => 'from-fuchsia-400', 'to' => 'to-rose-500',    'bg' => 'from-fuchsia-50/80 to-rose-50',  'ring' => 'ring-fuchsia-200', 'badge' => 'bg-fuchsia-100 text-fuchsia-800', 'icon' => 'heart'],
            ];
        @endphp
        <section>
            <div class="flex items-center gap-3 mb-6">
                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gradient-to-r from-sky-500 to-indigo-600 text-white text-xs font-extrabold uppercase tracking-wider shadow-md"><x-svg-icon name="pencil" class="w-3.5 h-3.5" /> {{ __('Content Editors') }}</span>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                @foreach ($editors as $i => $p)
                    ...
                    <article @click="author = @js($authorData($p, $colors[$i % count($colors)])); open = true"
                        ...
                        <span class="absolute top-3 right-3 text-gray-400 opacity-30 group-hover:opacity-100 group-hover:scale-125 transition"><x-svg-icon :name="$t['icon']" class="w-5 h-5" /></span>
                        ...
                        <div class="flex flex-wrap gap-1.5 justify-center items-center">
                            @if ($p->posts_count > 0)
                                <span class="inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 rounded-full {{ $t['badge'] }}"><x-svg-icon name="pencil" class="w-3 h-3" /> {{ $p->posts_count }}</span>
                            @endif
                            @if ($p->slug)
                                <a href="{{ route('author.show', $p->slug) }}" @click.stop
                                   class="inline-flex items-center text-[11px] font-bold px-2.5 py-1 rounded-full bg-white text-gray-700 hover:bg-gray-100 border border-gray-200 transition" title="{{ __('View profile') }}">
                                    <x-svg-icon name="newspaper" class="w-3 h-3" />
                                </a>
                            @endif
                            ...
                            @foreach ($socials as $type => $val)
                                ...
                                @php
                                    ...
                                    $iconName = ['linkedin' => 'briefcase','twitter' => 'chat-bubble','x' => 'chat-bubble','github' => 'computer','website' => 'globe','url' => 'globe','email' => 'envelope'][$lt] ?? null;
                                ...
                                @endphp
                                <a href="{{ $url }}" target="_blank" rel="noopener noreferrer me" @click.stop
                                   class="inline-flex items-center justify-center w-7 h-7 rounded-full bg-white/90 hover:bg-white border border-gray-200 text-gray-600 shadow-sm transition" title="{{ ucfirst($type) }}">
    ...
    @if ($mentors->isNotEmpty())
        <section>
            <div class="flex items-center gap-3 mb-6 flex-wrap">
                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gradient-to-r from-emerald-500 to-teal-600 text-white text-xs font-extrabold uppercase tracking-wider shadow-md"><x-svg-icon name="users" class="w-3.5 h-3.5" /> {{ __('Mentors') }}</span>
    ...
    @if (! empty($contributors) && $contributors->isNotEmpty())
        <section>
            <div class="flex items-center gap-3 mb-6 flex-wrap">
                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gradient-to-r from-amber-500 to-orange-600 text-white text-xs font-extrabold uppercase tracking-wider shadow-md"><x-svg-icon name="leaf" class="w-3.5 h-3.5" /> {{ __('Community Contributors') }}</span>
            ...
            <div class="flex flex-wrap gap-2.5">
                @foreach ($contributors as $i => $c)
                    ...
                    <div class="group flex items-center gap-2.5 bg-gradient-to-r {{ $pill }} border rounded-full pl-1.5 pr-3.5 py-1.5 hover:shadow-md hover:scale-105 transition cursor-default" title="{{ __('Community Contributor') }}">
                        ...
                        <span class="group-hover:rotate-12 transition"><x-svg-icon name="leaf" class="w-3.5 h-3.5" /></span>
                    </div>
                @endforeach
            </div>
        </section>
        ...
        <div class="relative grid md:grid-cols-[1fr_auto] gap-6 items-center">
            ...
            <div class="flex flex-wrap gap-3 shrink-0">
                <a href="/contribute" class="inline-flex items-center gap-2 bg-white text-indigo-700 hover:bg-indigo-50 font-bold px-5 py-3 rounded-xl shadow-lg transition">
                    <x-svg-icon name="pencil" class="w-4 h-4" /> {{ __('Share experience') }}
                </a>
                @if (\Illuminate\Support\Facades\Route::has('mentors.index'))
                    <a href="{{ route('mentors.index') }}" class="inline-flex items-center gap-2 bg-white/15 backdrop-blur-sm ring-1 ring-white/30 hover:bg-white/25 text-white font-bold px-5 py-3 rounded-xl transition">
                        <x-svg-icon name="users" class="w-4 h-4" /> {{ __('Become a mentor') }}
                    </a>
                @endif
            </div>
        </div>
    ...
    <div x-show="open" x-cloak
        ...
        <div x-show="open" x-transition
            ...
            {{-- yazıları --}}
            <div class="p-6 pt-4">
                <p class="text-xs font-bold uppercase tracking-wider text-gray-500 mb-3 inline-flex items-center gap-1.5"><x-svg-icon name="pencil" class="w-3.5 h-3.5" /> {{ __('Posts') }}</p>
    ...
    <section class="bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-2xl p-8 text-center">
        <h2 class="text-2xl font-extrabold mb-2 inline-flex items-center gap-2 justify-center"><x-svg-icon name="star" class="w-6 h-6 text-amber-300" /> {{ __('Contribute too') }}</h2>
        ...
        <div class="flex flex-wrap gap-3 justify-center">
            <a href="{{ route('contribute') }}"
               ...
               class="inline-flex items-center gap-2 bg-white text-indigo-700 hover:bg-gray-100 px-6 py-3 rounded-lg font-bold shadow-lg transition"><x-svg-icon name="leaf" class="w-4 h-4" /> {{ __('Share your experience') }}</a>
            <a href="{{ route('contribute') }}?role=editor"
               title="{{ __('Become a content editor') }} — technsug@gmail.com"
               class="inline-flex items-center gap-2 bg-white/15 backdrop-blur hover:bg-white/25 px-6 py-3 rounded-lg font-bold transition"><x-svg-icon name="pencil" class="w-4 h-4" /> {{ __('Become a content editor') }}</a>
            <a href="{{ route('mentors.index') }}"
               title="{{ __('Become a mentor') }}"
               class="inline-flex items-center gap-2 bg-white/15 backdrop-blur hover:bg-white/25 px-6 py-3 rounded-lg font-bold transition"><x-svg-icon name="users" class="w-4 h-4" /> {{ __('Become a mentor') }}</a>
        </div>
    </section>
</div>
@endsection
